...
- **우측 Command 한/영 토글이 특정 앱(특히 터미널 계열)에서 "아무 반응이 없다가 다른 곳을 클릭(refocus)해야 풀리던" 씹힘을 마저 고쳤습니다.** 1.15.3에서 이중 토글로 인한 즉시 원복은 시간 디바운스로 막았지만, 그 **억제 경로**에 모디파이어 상태 추적기 초기화가 빠져 있어 잔여 문제가 남아 있었습니다: 우측 modifier 물리 누름 1회가 `flagsChanged` press 이벤트 2개로 도착할 때 둘째 이벤트는 디바운스로 억제되는데, 이때 추적기의 command 비트를 비우지 않아, 토글이 입력 소스를 전환하는 순간 뒤따르는 key-up이 다른 컨트롤러로 배달되어 누락되면 그 비트가 눌린 채 고착(wedge)되고, 이후 모든 press가 "전이 없음"으로 상쇄되어 토글이 먹통이 되었습니다(포커스/모드 경계에서만 초기화되므로 refocus로만 복구). 발동 경로뿐 아니라 **억제 경로에서도** 추적기를 초기화하도록 고쳐 이 고착을 제거했습니다. 우측 토글 `flagsChanged` 판정은 순수 함수 `decideRightToggle`로 추출해, 이 회귀(억제-후-누락된-key-up → wedge)를 결정적으로 재현·검증하는 테스트를 추가했습니다. (`OSXCore/InputController.swift`, `OSXCore/InputMethodServer.swift`, `GureumTests/RightToggleKeyTests.swift`)
